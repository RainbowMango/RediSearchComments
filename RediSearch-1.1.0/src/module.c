...
int SetPayloadCommand(RedisModuleCtx *ctx, RedisModuleString **argv, int argc) {
  ...
  if (DocTable_SetPayload(&sp->docs, docId, md, mdlen) == 0) {
    RedisModule_ReplyWithError(ctx, "Could not set payload Â¯\\_(ãƒ„)_/Â¯");
    goto cleanup;
  }
...
FT.CREATE
   ...
   - query: the text query to search. If it's more than a single word, put it //query, ÊôÓÚËÑË÷ÒýÇæÊõÓïÖ®Ò»£¬±íÊ¾²éÑ¯µÄÄÚÈÝ£¬¿ÉÒÔÊÇ¹Ø¼ü´ÊÒ²¿ÉÄÜÊÇ¹Ø¼ü´ÊÁÐ±í
in
quotes.
...
overwritten //Ë÷ÒýÃû×Ö

    - NOOFFSETS: If set, we do not store term offsets for documents (saves memory, does not allow
      exact searches) //¸ÃÑ¡Ïî¾ö¶¨²»±£´æ´ÊÔÚÎÄµµÖÐµÄÆ«ÒÆÎ»ÖÃ£¬ÒÔ±ã½ÚÊ¡ÄÚ´æ¡£Ã»ÓÐÆ«ÒÆÎ»ÖÃ£¬Ò²¾ÍÎÞ·¨ÊµÏÖ¸ßÁÁ£¬±ÈÈç°Ù¶ÈËÑË÷½á¹ûÖÐµÄ¹Ø¼ü´Ê¸ßÁÁ
    ...
    - SCHEMA: After the SCHEMA keyword we define the index fields. They can be either numeric or
      textual. //SCHEMA ±ê¼ÇºóÐø²ÎÊý¶¼ÊÇÎÄµµ²ð·Ö³ÉµÄfield£¬¿ÉÒÔÊÇÊýÖµfieldÒ²¿ÉÒÔÊÇÎÄ±¾field¡£
      For textual fields we optionally specify a weight. The default weight is 1.0 //¶ÔÓÚÎÄ±¾field£¬È±Ê¡Ö¸¶¨1.0×÷ÎªÈ¨ÖØ
...
int CreateIndexCommand(RedisModuleCtx *ctx, RedisModuleString **argv, int argc) {
  ...
  if (argc < 5) {
    return RedisModule_WrongArity(ctx); //¼ì²é²ÎÊý¸öÊý£¬´´½¨Ë÷ÒýÖÁÉÙÐèÒª5¸ö²ÎÊý¡£ "FT.CREATE" "Ë÷ÒýÃû×Ö" "SCHEMA" "ÓòÃû×Ö" "TEXT" 
  }
  ...
  IndexSpec *sp = IndexSpec_CreateNew(ctx, argv, argc, &err); //¸ú¾ÝÃüÁî¸½´øµÄ²ÎÊý´´½¨Ò»¸öË÷Òý£¬·µ»ØË÷ÒýÖ¸Õë
  if (sp == NULL) {
...
int RediSearch_InitModuleInternal(RedisModuleCtx *ctx, RedisModuleString **argv, int argc) {
  ...
  RM_TRY(RedisModule_CreateCommand, ctx, RS_ADD_CMD, AddDocumentCommand, "write deny-oom", 1, 1, 1); //Ìí¼ÓÎÄµµÃüÁî FT.ADD
  ...
  RM_TRY(RedisModule_CreateCommand, ctx, RS_SEARCH_CMD, SearchCommand, "readonly", 1, 1, 1); //²éÑ¯ÃüÁî FT.SEARCH
